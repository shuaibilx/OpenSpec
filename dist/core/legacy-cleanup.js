...
function legacyToolIdForDir(dir) {
...
function legacyToolIdForFile(file) {
    ...
    for (const [toolId, pattern] of Object.entries(LEGACY_SLASH_COMMAND_PATHS)) {
        if (pattern.type !== 'files' || !pattern.pattern)
            continue;
        const patterns = Array.isArray(pattern.pattern) ? pattern.pattern : [pattern.pattern];
        if (patterns.some((p) => globToRegex(p).test(normalizedFile)))
            return toolId;
    }
    return undefined;
}
 ...
 */
export function omitToolLegacyArtifacts(detection, toolIds) {
    if (toolIds.length === 0)
        return detection;
    const skip = new Set(toolIds);
    const nextDetection = {
        ...detection,
        slashCommandDirs: detection.slashCommandDirs.filter((dir) => !skip.has(legacyToolIdForDir(dir) ?? '')),
        slashCommandFiles: detection.slashCommandFiles.filter((file) => !skip.has(legacyToolIdForFile(file) ?? '')),
    };
    nextDetection.hasLegacyArtifacts = hasLegacyArtifacts(nextDetection);
    return nextDetection;
}
