...
import { classifyOpenSpecDir, MAX_CONTEXT_SIZE, readProjectConfig, storePointerProblem, } from './project-config.js';
...
function formatLanguageContext(language) {
    return [
        `Language: ${language}`,
...
export class InitCommand {
        ...
        if (guardRoot) {
            ...
            if (!hasPlanningShape) {
                if (pointer.malformed) {
                    throw new Error(`The store declaration in ${pointer.filePath} is invalid (` +
                        storePointerProblem(pointer.malformed) +
                        `). Fix or remove the store: line before running openspec init.`);
                }
                if (pointer.value !== undefined) {
                    throw new Error(`This repo's planning is externalized to store '${pointer.value}' (${pointer.filePath}). ` +
    ...
    validateTools(toolIds, toolStates, projectPath) {
        ...
        const reconciledToolIds = sharedTargetCandidates.length > 1
            ? toolIds.flatMap((toolId, index) => {
                if (!sharedAgentsTargets.includes(toolId))
                    return [toolId];
        ...
        if (reconciledToolIds.length !== toolIds.length ||
            ...
            console.log(chalk.dim(`Codex, Zed, and agents share .agents/skills; writing one tree for ${sharedTargetOwner}.`));
        }
    ...
    normalizeLanguage(language) {
        ...
        if (/\p{Cc}|\p{Bidi_Control}|[\u200B\u2028\u2029\uFEFF]/u.test(normalized)) {
            throw new Error('The --language option must be a single line without control or invisible formatting characters.');
        }
        const serializedContext = `${formatLanguageContext(normalized)}\n`;
        if (Buffer.byteLength(serializedContext, 'utf8') > MAX_CONTEXT_SIZE) {
            throw new Error(`The --language option is too long for OpenSpec's ${MAX_CONTEXT_SIZE / 1024}KB project context limit.`);
        }
        return normalized;
    }
    languageContext() {
    ...
    async assertLanguageCanBeApplied(projectPath, openspecPath) {
        ...
        if (!hasConfig) {
            ...
            if (!(await FileSystemUtils.canWriteFile(configPath))) {
                throw new Error('Cannot create openspec/config.yaml for --language: the destination is not writable.');
            }
            return;
        }
        const existingContext = readProjectConfig(projectPath)?.context;
        if (existingContext?.includes(languageContext))
            return;
        throw new Error('--language does not overwrite an existing OpenSpec config. ' +
    ...
    displaySuccessMessage(projectPath, tools, results, configStatus, copilot) {
        ...
        const restartCommandsGenerated = successfulTools.some((tool) => tool.requiresIdeRestart &&
            shouldGenerateCommandsForTool(tool.value, activeDelivery));
        const restartSkillsGenerated = successfulTools.some((tool) => tool.requiresIdeRestart &&
            shouldGenerateSkillsForTool(tool.value, activeDelivery));
        if (restartCommandsGenerated || restartSkillsGenerated) {
            console.log();
            console.log(chalk.white(restartCommandsGenerated
