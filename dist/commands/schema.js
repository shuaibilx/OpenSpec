...
export function registerSchemaCommand(program) {
    ...
    schemaCmd
        ...
        try {
            ...
            parseSchema(fs.readFileSync(path.join(trustedSourceDir, 'schema.yaml'), 'utf-8'));
            // Check destination
            ...
            if (resolvedDestination === trustedSourceDir) {
                throw new Error(`Cannot fork schema '${source}' onto itself; choose a different destination name`);
            }
            ...
            try {
                ...
                try {
                    ...
                    throw new Error(`The staged fork of '${source}' is not a valid schema (the source may have changed during copy); ` +
                        `aborted, '${destinationName}' was not modified.`, { cause: validationError });
                }
                ...
                if (destinationExists) {
                    if (spinner)
                        spinner.text = `Replacing existing schema '${destinationName}'...`;
                    ...
                    if (currentFingerprint !== authorizedDestinationFingerprint) {
                        throw new Error(`Schema '${destinationName}' at ${destinationDir} changed on disk while the fork was being prepared. ` +
                    ...
                    try {
                        ...
                        try {
                            ...
                            throw new Error(`Failed to install the forked schema and could not restore the previous '${destinationName}'. ` +
                                `Your previous schema is preserved at ${backupDir}; move it back to ${destinationDir} to restore. ` +
                                `Restore error: ${restoreError.message}`, { cause: installError });
                        }
                        throw installError;
                    }
                    ...
                    if (fingerprintDir(backupDir) === authorizedDestinationFingerprint) {
                        ...
                        console.error(`Warning: the previous '${destinationName}' changed during the fork and was NOT deleted; ` +
