...
**Profile.** The set of slash commands installed in your project. **Core** (the default) is `roadmap`, `propose`, `explore`, `apply`, `update`, `sync`, `archive`. The **expanded** set adds `new`, `continue`, `ff`, `verify`, `bulk-archive`, `onboard`. Change it with `openspec config profile`.
