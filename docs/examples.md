...
Real changes, start to finish. Each recipe shows the commands you'd type and what you'd see back, so you can match your situation to a pattern and copy it. These use the default **core** commands (`roadmap`, `propose`, `explore`, `apply`, `update`, `sync`, `archive`); where the expanded set helps, it's noted.
