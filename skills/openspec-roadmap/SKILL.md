...
**Input**: The argument after `/openspec-roadmap` is the broad initiative to plan, an existing roadmap to reconcile, or a roadmap/status question. Examples:
...
- hand off to `/openspec-explore` when material decisions remain, or `/openspec-propose` when the scope is already clear;
...
Do not estimate dates or effort unless the user asks. Do not pre-design later phases: each phase begins with a fresh `/openspec-explore` against the code and specs that exist then.
...
- recommend a fresh `/openspec-explore <next-change-id>` for the next phase, or `/openspec-propose <next-change-id>` only when no material decisions remain;
